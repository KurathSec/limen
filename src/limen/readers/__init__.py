...
Release 1 reads the generic long CSV and lm-evaluation-harness ``--log_samples``
trees. Deliberately absent, stated rather than silent (spec ruling LMN-CORE-006):
inspect_ai ``.eval`` per-epoch logs, HELM, and Parquet — each is one new module
and one tuple entry when it earns its cost.
"""
...
READERS: tuple[Reader, ...] = (LongCsvReader(), LmEvalReader())
...
def _resolve(path: Path, format: str | None) -> Reader:
    ...
    if not claims:
        raise ReaderError(
            ...
            "from --log_samples; pass --format to override"
        )
    raise ReaderError(
