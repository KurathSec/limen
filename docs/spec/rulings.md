...
Rulings spec version: **0.1.0**. A ruling is never edited to mean
...
*Status: active.* Readers deliberately absent from this release (inspect_ai .eval per-epoch, HELM, Parquet) are named in the reader registry and the docs; their absence is a scope decision, not an accident.
