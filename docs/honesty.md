...
5. **It does not show the stable-items-only ranking is the true ranking**
   ...
   view, not a correction — and it is never emitted without its
   selection-bias mitigations (disjoint draw splits and a selection null).
...
population as that project's own reporting, not claims about, or corrections
to, any table published from that ladder elsewhere, and never statements about
which of the four models is better.
